...
@Component({
  selector: 'app-empleadoimporte',
  templateUrl: './empleadoimporte.component.html',
  styleUrls: ['./empleadoimporte.component.css']
})
export class EmpleadoimporteComponent {
  element: any;
  token: string;
  afiliado: any;
  aportes: any;
  ...
  ngOnInit():void{
    ...
    this.obtener_importe();
  }

  public obtener_importe(){
    this.emp_servicio.obtener_empleados_importe(this.token,this.element[0].emp_cuit,this.element[1])
    .subscribe((data: any) => {
      ...
      this.dataSource.data = data.map((item: any) => {
        ...
        item.importes.forEach((importe: any, index: number) => {
          newItem[this.months[index]] = parseFloat(importe.ap_total).toFixed(2);
        });
        return newItem;
      });
